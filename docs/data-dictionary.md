...
- **Soft delete:** most record tables aren't hard-deleted. `notes`, `comms`,
  ...
  "trash bin" — non-null means deleted). `sensors` use an `active` flag
...
- `id` — the pod label. **3-digit pods are `MOD-00xxx`; 4-digit pods are
  `MOD-X-PM-0xxxx`** (the "X-PM" part is significant — keep it).
...
- `location_id` — **the community/lab where it happened** (note: this column is
  named `location_id`, not `community_id` — the one naming inconsistency).
...
### `allowed_emails`
The signup allow-list — only emails here can create an account. Enforced by a
