...
-- Unschedule any remaining QuantAQ cron jobs (pg_cron). Plain SQL — calls
-- cron.unschedule once per matching job; a no-op (0 rows) if none remain.
SELECT cron.unschedule(jobid) FROM cron.job WHERE jobname LIKE 'quantaq%';
